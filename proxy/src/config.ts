...
function normalizeLogDir(value: string): string {
  if (
    value === "proxy/logs" &&
    new URL(".", import.meta.url).pathname.endsWith("/proxy/src/")
  ) {
    return "logs";
  }
  return value;
...
function parseApiKeys(envVar: string, fileEnvVar: string): string[] {
  ...
  if (fileVal) {
    try {
      ...
      console.warn(
        `[Config] Failed to read keys from ${fileVal}: ${(e as Error).message}`,
      );
    }
  }
  return [...new Set(keys)];
...
function loadTomlConfig(path: string): {
  ...
  try {
    ...
    if (data.model_providers && typeof data.model_providers === "object") {
      for (const [id, provider] of Object.entries(data.model_providers)) {
        if (provider && typeof provider === "object") {
          ...
          channels[id] = {
            baseUrl: p.base_url || "",
            apiKeys: Array.isArray(p.api_keys)
              ? p.api_keys
              : (p.api_key ? [p.api_key] : []),
            model: p.model,
            modelMapping: p.model_mapping && typeof p.model_mapping === "object"
              ? p.model_mapping
              : undefined,
          };
        }
    ...
    return {
      ...
      expertChannel: data.expert_provider,
      channels,
      modelMapping: data.model_mapping && typeof data.model_mapping === "object"
        ? data.model_mapping
        : undefined,
    };
  } catch (e) {
    if (!(e instanceof Deno.errors.NotFound)) {
      console.warn(
        `[Config] Failed to load TOML config from ${path}: ${
          (e as Error).message
        }`,
      );
    }
...
export function loadConfig(): ProxyConfig {
  ...
  for (const [key, value] of Object.entries(Deno.env.toObject())) {
    if (key.startsWith("MODEL_MAP_")) {
      const logicalName = key.slice("MODEL_MAP_".length).toLowerCase().replace(
        /_/g,
        ".",
      );
  ...
  const config: ProxyConfig = {
    ...
    expertChannel: tomlData.expertChannel || env("EXPERT_CHANNEL", ""),
    ...
    sanitizeUpstreamPrompts: envBoolean(
      "OPENAI_SANITIZE_UPSTREAM_PROMPTS",
      false,
    ),
    augmentModelContextTokens: envNumber(
      "AUGMENT_MODEL_CONTEXT_TOKENS",
      200000,
    ),
    augmentModelMaxOutputTokens: envNumber(
      "AUGMENT_MODEL_MAX_OUTPUT_TOKENS",
      16000,
    ),
    ...
    augmentHistorySummaryPrompt: env(
      "AUGMENT_HISTORY_SUMMARY_PROMPT",
      defaultHistorySummaryPrompt,
    ),
    ...
    embedBaseUrl: normalizeEmbedBaseUrl(
      env("EMBED_BASE_URL", "http://127.0.0.1:11434"),
    ),
  ...
  if (switchApi === "OPENAI" && Object.keys(config.channels).length === 0) {
    throw new Error(
      "No channels configured (OPENAI_API_KEY or PROXY_CONFIG_FILE)",
    );
  }
  if (switchApi === "CODEX") {
    if (!config.codexBaseUrl) {
      throw new Error("CODEX_BASE_URL is required when SWITCH_API=CODEX");
    }
    if (!config.codexApiKey) {
      throw new Error("CODEX_API_KEY is required when SWITCH_API=CODEX");
    }
    if (!env("CODEX_MODEL")) {
      throw new Error("CODEX_MODEL is required when SWITCH_API=CODEX");
    }
  }
...
export function getCurrentChannel(config: ProxyConfig): ChannelConfig | null {
  return config.channels[config.activeChannel] || config.channels["default"] ||
    null;
}

export function getExpertChannel(config: ProxyConfig): ChannelConfig | null {
  if (!config.expertChannel) return null;
  return config.channels[config.expertChannel] || null;
}

let nextOpenaiKeyIndex = 0;
export function getNextOpenaiApiKey(
  config: ProxyConfig,
  channel?: ChannelConfig | null,
): string {
  channel = channel ?? getCurrentChannel(config);
  ...
  const key =
    config.openaiApiKeys[nextOpenaiKeyIndex % config.openaiApiKeys.length];
  nextOpenaiKeyIndex++;
  return key;
}
...
export function getNextEmbedApiKey(config: ProxyConfig): string {
  ...
  const key =
    config.embedApiKeys[nextEmbedKeyIndex % config.embedApiKeys.length];
  nextEmbedKeyIndex++;
  return key;
}
